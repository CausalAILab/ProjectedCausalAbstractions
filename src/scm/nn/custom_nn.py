...
from .resnet import MLP_ResNet_Module
...
class CustomNN(nn.Module):
    def __init__(self, pa_size, u_size, o_size, pa_type, o_type, img_size=32,
                 img_embed_size=4, feature_maps=64, h_size=64, h_layers=3, groups=2,
        ...
        if use_mlp:
            ...
            if use_deconv:
                mlp_out_size = h_size
            if self.mode == "resnet_disc":
                self.mlp_mod = MLP_ResNet_Module(self.real_i_size, mlp_out_size, groups=groups,
                                                 width_per_group=h_size, group_layers=h_layers,
    ...
    def forward(self, pa, u, include_inp=False):
        ...
        if len(u.keys()) == 0 and len(self.real_pa) == 0:
            ...
            elif len(pa.keys()) == 0 or len(set(pa.keys()).intersection(self.real_pa)) == 0:
