        ...
        if (self.current_epoch + 1) % 10 == 0:
            if not self.logged:
                ...
                if self.task == "sampling":
                    ...
                    q_estimate = self.datagen.datagen.calculate_query(model=self.ncm, tau=self.use_tau, m=10000,
                                                                      evaluating=True)
                    q_true = self.datagen.datagen.calculate_query(model=None, tau=self.use_tau, m=10000,
                                                                  evaluating=True)
